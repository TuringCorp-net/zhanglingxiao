...
| 方法 | 路径 | 模块 | SRS | 状态 |
|------|------|------|-----|------|
| POST | `/api/write/conflicts/generate` | `conflicts.ts` | SF-024 | ✅ |
| GET | `/api/write/conflicts/{work_id}` | `conflicts.ts` | SF-024 | ✅ |
...
**Write 侧端点总数**：31 个（+9 工作区 +4 世界观 +2 伏笔 +2 冲突 +3 大纲 +6 流水线 +3 营销 +3 状态转换）
...
| SF-060 | 软木板视图 | ✅ 已实现（含拖拽排序和状态筛选） |
| SF-061 | 写作桌视图 | ✅ 已实现（三栏布局+Markdown 编辑/预览+四 Tab 右面板） |
...
| 前端范围 | 写作桌+软木板双模式 | 双模式已实现 | 2026-05-08 完成 SF-061/062 |

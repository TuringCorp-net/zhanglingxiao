...
| v2.3.1 | 2026-05-19 | 槽位标记格式升级：`<!-- -->` 单标记 → `<!-- slot:hint -->...<!-- /slot -->` 成对标记，解析器不再需要猜测槽位边界。AI prompt 更新为"在标记之间写入内容，保留标记" |
...
**总计**：45 项需求（42 已实现 + 2 待实现 + 1 已移除）。待实现：SF-056（Read 侧 AI 伴读后端）、SF-063（写作引导流程）。已移除：SF-024（冲突地图）。
...
| 写作桌 UI | SF-060~066 | ❌ SF-060（已合并）, ✅ SF-061~062, ⏳ SF-063, ✅ SF-064~066 |

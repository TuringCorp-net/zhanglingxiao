  ...
  <nav class="top-nav">
    <div class="container">
      ...
      <div class="nav-right">
        ...
        <button class="login-btn" id="nav-login-btn" onclick="${isWrite ? `var tk=prompt(t('prompt.token_debug'),typeof userToken!=='undefined'?userToken:'');if(tk){localStorage.setItem('sf_user_token',tk);location.reload()}` : `location.href='/connect.html'`}" data-i18n="nav.login">${t('nav.login')}</button>
      </div>
    </div>
  </nav>`;
}

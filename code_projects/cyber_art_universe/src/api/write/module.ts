...
// ============================================================
// M5 Intent 数据迁移：旧嵌套 JSON → {slots:{...}} 格式
// POST /api/write/admin/migrate-intents
// ============================================================

/** 将旧格式 M5 intent JSON 转换为新格式 {slots:{...}} */
...
export async function migrateM5Intents(env: Env, _request: Request): Promise<Response> {
  // 查询所有 m5_intent 模块
  const mods = await env.DB.prepare(
    "SELECT id, work_id, r2_json_key FROM modules WHERE type = 'm5_intent'"
  ).all<{ id: string; work_id: string; r2_json_key: string | null }>();

  if (!mods.results?.length) {
    return new Response(JSON.stringify(jsonSuccess({ migrated: 0, message: 'No M5 intent modules found' })), {
      headers: { 'Content-Type': 'application/json' },
    });
  }
  ...
  for (const mod of mods.results) {
    if (!mod.r2_json_key) { skipped++; continue; }

    // 尝试 zh 和 en 两种语言
    for (const lang of ['zh', 'en'] as Lang[]) {
      try {
        const key = workContentPath(mod.work_id, lang, mod.r2_json_key);
        const obj = await env.WORKS_BUCKET.get(key);
        if (!obj) continue;
        ...
        // 旧格式：嵌套 JSON → {slots:{...}}
        const slots = intentToSlots(raw);
        if (Object.keys(slots).length === 0) {
          skipped++;
          continue;
        }

        const newData: R2SlotData = { slots };
        await env.WORKS_BUCKET.put(key, JSON.stringify(newData, null, 2), {
          httpMetadata: { contentType: 'application/json' },
        });

        migrated++;
        details.push(`${mod.id} (${lang}): ${Object.keys(slots).length} slots migrated`);
      } catch (err) {
        details.push(`${mod.id} (${lang}): ERROR - ${String(err)}`);
      }
    }
  }
  ...
  return new Response(JSON.stringify(jsonSuccess({
    migrated,
    skipped,
    total_modules: mods.results.length,
    details,
  })), {
    headers: { 'Content-Type': 'application/json' },
  });

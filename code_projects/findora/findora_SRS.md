...
> **版本：** v3.31（架构基线重构：对齐 business_concept v1.1）
> **最后更新：** 2026-04-13
...
| 修改时间 | 修改内容 |
|----------|----------|
| 2026-04-13 | 删除 products 表重复字段定义（P0-1 修复）；修正 Cron 口径由"每周五 10:00"改为"每周四 09:00"（R3 修复）；同步 Business Concept v1.1 架构重构（用户侧零实时 LLM、外部运营 AI 异步化、纯数据库推荐链路） |
...
| v3.30 | 2026-04-13 | STR基线维护 | 历史版本，详见 `findora_STR.md` |
...
**统计口径说明**：
- 若仅统计站内 API（公共+用户+管理+配置+认证），当前共 **25** 个端点。
- 若包含外部系统集成接口（F-040-20~23），当前共 **29** 条接口定义。
- F-040-19 为预留编号，暂未启用。
...
### 3.1.8 架构一致性检查清单（v3.31）

| 编号 | 检查项 | 验收标准 | 关联功能 |
|------|--------|----------|----------|
| AC-01 | 用户侧是否存在实时 LLM 调用 | Web 请求链路中 0 次外部模型调用 | F-016/F-020 |
| AC-02 | 运营AI入库是否必须鉴权 | 无 Token 请求全部拒绝（401/403） | F-040-22 |
| AC-03 | 标签维度是否支持动态扩展 | 新维度可通过接口创建并立即用于检索 | F-011/F-040-22 |
| AC-04 | 推荐是否为纯查库逻辑 | 推荐链路仅含 DB 检索 + 随机抽选 | F-014/F-015/F-016 |
| AC-05 | API 是否仍为唯一数据入口 | 无前端/Agent 直连 D1/R2路径 | F-040/S-09 |
...
| 编号 | 描述 | 当前状态 | 推进建议 |
|------|------|----------|----------|
| F-040-22 | 运营AI数据更新接口（API/CLI） | 🗓 | 先落地 Token 鉴权、schema 校验、幂等与 dry-run |

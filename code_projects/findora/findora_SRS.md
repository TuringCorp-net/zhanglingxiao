...
| 2026-05-03 | v5.46：Reviewer定时任务；Code Review完成；代码基线稳定；四文档v5.46同步 |
...
**Coder定时任务：四文档版本对齐；无P0/P1阻塞项；更新日志清理。**

1. ✅ **版本更新**：四文档 v5.46→v5.47 统一
2. ✅ **更新日志清理**：按规则仅保留最新3天内容
3. ✅ **TypeScript编译检查**：`npx tsc --noEmit` 0错误
4. ✅ **四文档版本对齐**：SRS/SDS/API/STR版本统一为v5.47
5. ✅ **Business Concept约束**：A-01~A-06全部满足
6. ✅ **系统设计约束**：A-01~A-06全部通过
...
| **A-02** | 外部运营AI异步化：AI 作为系统外运营角色，仅通过数据更新接口推送结果 | F-020/F-040-22 |
| **A-03** | 纯数据库推荐链路：推荐流基于 `用户标签矩阵 ∩ 商品标签矩阵` 检索后随机抽选 | F-014/F-015/F-016 |
| **A-04** | 动态标签维度：标签维度与标签项均可动态新增，禁止硬编码固定维度 | F-011/F-040-22 |
| **A-05** | 统一数据 API 层：前端与 Agent 不得直连 D1/R2，所有读写走 API 层 | F-040/S-09 |
| **A-06** | Cloudflare 优先：Workers + D1 + R2 为主技术栈，避免引入非必要外部基础设施 | 技术选型 |
...
>
> **示例**：F-016 的"需求设计"列为✅表示需求文档已完成，"代码实现"列为🏗表示代码已实现但功能审核尚未完成。待AI服务API Key联调后，将功能审核状态从🗓推进至✅。最终用户看到的推荐解释100%来自预生成文案检索，无需实时AI。
...
| F-040-22 运营AI数据更新接口 | ✅ | 🏗 | 🗓 | upsert端点已实现（`POST /api/admin/products`）；幂等保证（ai_update_logs表）已通过迁移022实现 |
...
| 版本 | 日期 | 完成模块 | 备注 |
|------|------|----------|------|
| v5.47 | 2026-05-04 | Coder定时任务 | 四文档版本对齐；无P0/P1阻塞项 |
| v5.46 | 2026-05-03 | Reviewer定时任务 | Code Review完成；代码基线稳定 |
| v5.45 | 2026-05-02 | Coder定时任务 | 四文档v5.45同步；代码基线稳定 |
...
> **代码实现状态**：✅ `POST /api/admin/products`端点实现了upsert能力（含幂等），`ai_update_logs`表及幂等校验逻辑已通过迁移022实现。
...
- F-025是人工干预机制的整体功能模块，包括F-021审核结果的执行、以及非AI生成内容的其他人工干预
...
### 10.4 运营Agent协作流程映射（v3.86新增）

> **需求来源**：system_design.md §二"运营Agent团队"、business_concept运营流程

根据 system_design.md，运营团队由 **Selector**、**Curator**、**Operator** 三个Agent组成，通过文件夹结构完成协作闭环。

#### 运营Agent职责与SRS功能映射

| 运营Agent | 主要职责 | 对应SRS功能 | 数据流向 |
|-----------|----------|-------------|----------|
...
```
[1. Selector Agent]
  输入：1688/TEMU/Amazon/速卖通/TikTok Shop 各平台原始商品
  工具：JJY API（operations/tools/jjy_api.js，5平台128品类，免登录）
  输出：候选商品列表（/selected/，每个商品一个编号+markdown文件）
        │
        ▼
[2. Curator Agent]
  输入：/selected 文件夹下的商品
  处理：分析商品特征 → 策划推广文案（为什么有趣、尝试理由）
  输出：推广文案草稿（/candidate/，每个商品一个编号+markdown文件）
        │
        ▼
[3. Operator Agent]
  输入：/candidate 文件夹下的商品
  处理：逐个决策是否具备爆款潜力
  输出：
    ✅ 通过 → 归档至 /pass → 调用 F-040-22 或 F-040-14/15 入库
    ❌ 拒绝 → 归档至 /notpass（仅留存，不上架）
```
...
| 运营操作 | 触发接口 | SRS功能 | 说明 |
|----------|----------|---------|------|
| 批量商品入库 | `POST /api/admin/ai/upsert` | F-040-22 | 运营AI数据更新接口，含幂等、校验、Token鉴权 |
| 单个商品新增 | `POST /api/admin/products` | F-040-14 | 手动新增商品（人工审核后） |
| 单个商品编辑 | `PUT /api/admin/products/:id` | F-040-15 | 修改商品信息 |
| 标签管理 | `POST /api/admin/tags` | F-040-17 | 动态标签维度与标签项管理 |
| AI内容审核 | `POST /api/admin/ai-review/create` | F-021 | AI生成内容审核工作流 |
| 推荐干预 | `PUT /api/admin/configs/:key` | F-040-25 | 推荐权重调整、商业排序配置 |
...
project_root/
├── operations/                    # 运营工作区
│   ├── selected/                  # Selector产出（候选商品原始数据）
│   │   ├── 001_product_xxx.md
│   │   └── 002_product_yyy.md
│   ├── candidate/                 # Curator产出（推广文案草稿）
│   │   ├── 001_product_xxx.md
│   │   └── 002_product_yyy.md
│   ├── pass/                      # Operator通过（待上架或已上架）
│   │   └── 001_product_xxx.md
│   └── notpass/                   # Operator拒绝（仅归档）
│       └── 002_product_yyy.md
└── docs/
    ├── SRS.md                     # 由Designer产出
    ├── API_Reference.md            # 由Coder维护
    └── STR_Report.md              # 由Reviewer产出
```

#### 与business_concept §11对齐

- **运营AI定位**：运营Agent（Selector/Curator/Operator）是系统外的"外部运营AI"角色，不是用户请求链路中的实时LLM
- **数据入库要求**：所有运营Agent产出必须通过 F-040-22 接口入库，禁止旁路写库
- **人工控制权**：Operator最终决策权属于人类，F-021 AI审核工作流是辅助工具，不替代人工判断
...
> **v5.00 执行基线**：按”用户侧零实时LLM + 外部运营AI异步入库 + 纯查库推流”推进。

### 待推进项（按优先级）

| 编号 | 描述 | 当前状态 | 推进建议 |
|------|------|----------|----------|
| 1 | 外部运营AI服务接入规范 | 🗓 | Token鉴权已实现；契约已完善；F-040-22已实现幂等保证 |
| 2 | JJY API选品工具落地 | 🗓 | operations/tools/jjy_api.js已实现；需Selector Agent集成 |
| 3 | 本地E2E验证 | 🗓 | 执行001~022迁移脚本 |
| 4 | 端到端链路测试 | 🗓 | Postman/HTTP验证核心流 |
| 5 | 动态标签维度治理 | 🗓 | 增加维度生命周期管理与冲突检测 |
...
### business_concept.md 17章节映射核对清单（v5.00）
...
### system_design.md v1.1.0 架构约束同步核对清单（v5.00）

...
let products = Array.isArray(rawData) ? rawData : (rawData.products || []);
...
console.log(`去重后商品数: ${products.length}`);
...
const INTEREST_KEYWORDS = [
  'funny', 'quirky', 'unique', 'weird', 'cute', 'novelty', 'bizarre',
  'creative', '趣味', '搞怪', '创意', '新奇', '可爱', '独特',
  'multi-function', '2-in-1', '3-in-1', 'all-in-one', 'portable', 'foldable',
  '多功能', '便携', '折叠', '迷你', 'DIY', 'mini',
  'decor', 'gift', 'party', 'holiday', 'christmas', 'halloween',
  '装饰', '礼品', '派对', '节日', '礼物',
  'led', 'rgb', 'wireless', 'bluetooth', 'smart', 'automatic',
  'LED', '智能', '自动',
  'kitchen', 'bedroom', 'bathroom', 'office', 'storage', 'organizer',
  '厨房', '卧室', '收纳', '整理',
  'outdoor', 'camping', 'travel', 'hiking',
  '户外', '露营', '旅行',
...
function scoreProduct(p) {
  ...
  if (price >= 5 && price <= 50) score += 15;
  else if (price > 0 && price < 5) score += 5;
  else if (price > 50 && price <= 100) score += 8;

  // 评论数适中加分
  const reviews = p.reviewNum || 0;
  if (reviews >= 100 && reviews <= 5000) score += 15;
  else if (reviews > 0 && reviews < 100) score += 10;
  ...
  // 销量适中加分
  const sold = p.sold || 0;
  if (sold >= 500 && sold <= 50000) score += 10;
  else if (sold > 0 && sold < 500) score += 8;
  else if (sold > 50000 && sold <= 200000) score += 3;

  // 图片质量加分
  if (p.thumbnail && p.thumbnail.length > 0) score += 5;
...
for (const p of scored) {
  ...
  if (!platformCount[platform]) platformCount[platform] = 0;
  if (platformCount[platform] < platformLimit) {
    selected.push(p);
...
for (let i = 0; i < Math.min(15, scored.length); i++) {
  ...
  console.log(`   ${(p.goodsNameEn || 'N/A').substring(0, 70)}...`);
}

console.log('\n===== 入选商品 =====');
selected.forEach((p, i) => {
  console.log(`${i + 1}. [${p.platform}] ${(p.goodsNameEn || 'N/A').substring(0, 50)}`);
});
...
- **原始商品数**: ${rawData.length || uniqueProducts.length}
- **去重后**: ${uniqueProducts.length}
- **筛选标准**: 新奇/有趣/好玩/有爆点
...
for (let i = 0; i < selectedProducts.length; i++) {
  ...
  const linkBase = p.platform === 'temu' ? 'temu.com' :
                   p.platform === 'shein' ? 'shein.com' :
                   p.platform === 'amazon' ? 'amazon.com' :
                   p.platform === 'sumaitong' ? 'aliexpress.com' :
                   'tiktok.com';
...
- **缩略图**: ${p.thumbnail || 'N/A'}
- **商品链接**: ${p.goodsId ? `https://www.${linkBase}/item/${p.goodsId}.html` : 'N/A'}
...
// 保存选中商品JSON
const selectedJsonPath = path.join(__dirname, 'selected_20260428_final.json');
...
## 原始链接
${p.goodsId ? `https://www.${linkBase}/item/${p.goodsId}.html` : 'N/A'}
`;
